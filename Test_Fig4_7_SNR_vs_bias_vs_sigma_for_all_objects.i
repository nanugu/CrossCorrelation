 ...
 */
{
  ...
  datafile= "Fits/"+object+"_SNR.fits";
  ...
  SolarFlux=spanl(2000.0, 1e7, N); /* Flux varying solar */
  ...
  for(i=1; i<=N; i++){//for each shift
    ...
    if(object=="LGS"){
      ...
      shx= shy=0.25; //solar 
    }else shx=shy=0.25; //point, GC, Solar
    ...
    for(j=1; j<=NR; j++){//for reach random realization
      ...
      if(object=="solar"){
          tmp= IM_center(reference, image_noise, 7, 1, 1);
      }else{
          tmp= IM_center(reference, image_noise, 1, 1, 1);
      }
      
      RPyramid(,j)= PyramidApp(PeakIM)(1:2) - Nx/2.;//subtract Nx/2 to have 0 instead of 8 pix
      RParabola(,j)= ParabolaApp(PeakIM)(1:2) - Nx/2.;
      REquiangular(,j)= Equiangular_line(PeakIM)(1:2) - Nx/2.;
      RGaussData(,j)= GaussApp(PeakIM)(1:2) - Nx/2.;
      RQuadraticInterpolationD(,j)= QIApp(PeakIM)(1:2) - Nx/2.;
      RCoG(,j)= CoGApp(PeakIM)(1:2) - Nx/2.;
      ROCoG(,j)= CoGApp(image_noise) - Nx/2.;
...
*/
    ...
    write, i, shx, CoGApp(PeakIM)(1)-Nx/2.0-shx; 
  }
  ...
  Data(,4)= OCoG(1,)-IN(1,);                      //direct application on spot
